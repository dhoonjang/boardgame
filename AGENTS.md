# AGENTS.md

This file provides guidance to Codex (Codex.ai/code) when working with code in this repository.

## 중요: 게임 규칙 문서화

**게임 로직 관련 작업 시 반드시 준수할 것:**

1. **작업 전**: 해당 게임의 `AGENTS.md`를 먼저 읽어서 현재 규칙을 파악
2. **새로운 규칙 발견 시**: 사용자가 알려준 규칙이나 구현하면서 알게 된 규칙을 즉시 `AGENTS.md`에 반영
3. **혼동 방지**: 규칙이 불명확하거나 기존 문서와 다르면 사용자에게 확인 후 문서 업데이트

예시: forgod 게임 작업 시 → `packages/forgod-server/AGENTS.md` 확인 및 업데이트

## 프로젝트 개요

보드게임 프로젝트 모노레포. 여러 보드게임을 패키지로 관리합니다.

## 모노레포 구조

```
boardgame/
├── packages/
│   ├── <game>/           # 게임별 웹 UI
│   ├── <game>-core/      # 게임별 로직 라이브러리 (순수 TypeScript) — 선택
│   └── <game>-server/    # 게임별 서버 (게임 로직 포함 가능)
├── pnpm-workspace.yaml
└── turbo.json
```

### 현재 게임
- **forgod**: For God - 신과 마왕 사이에서 운명을 선택하는 3~6인용 전략 보드게임 (server + UI, 게임 로직은 server 내부)
- **indian-poker**: Indian Poker - 캐릭터 AI와 함께하는 2인 인디언 포커 카드 게임 (server + UI, 게임 로직은 server 내부)

### indian-poker 규칙 메모
- 첫 베팅 턴에서 `CALL`(체크)은 허용하지 않음. 선 플레이어는 `RAISE` 또는 `FOLD`만 가능.

## 패키지 구조 패턴

게임별로 구조가 다를 수 있음:

**forgod** (서버 통합형):
| 패키지 | 역할 | 기술 스택 |
|--------|------|-----------|
| `forgod` | 웹 UI | Vite + React + React Router + Zustand + Tailwind |
| `forgod-server` (= `@forgod/server`) | HTTP API 서버 + 게임 로직 | Hono + @hono/node-server + Zod + TypeScript |

forgod 의존성: `forgod` → `@forgod/server/game` (타입만, vite alias로 소스 직접 참조)

**indian-poker** (서버 통합형):
| 패키지 | 역할 | 기술 스택 |
|--------|------|-----------|
| `indian-poker` | 웹 UI | Vite + React + Zustand + Tailwind |
| `indian-poker-server` (= `@indian-poker/server`) | 서버 + 게임 로직 + AI 캐릭터 | Socket.IO + Hono + Zod + Anthropic SDK |

indian-poker 의존성: `indian-poker` → `@indian-poker/server/game` (타입만, vite alias로 소스 직접 참조)

## 공통 명령어

```bash
# 의존성 설치
pnpm install

# 전체 프로젝트 (Turborepo)
pnpm dev                 # 모든 패키지 dev 실행
pnpm build               # 모든 패키지 빌드
pnpm test                # 모든 패키지 테스트
pnpm lint                # 모든 패키지 린트

# 특정 패키지 명령어 실행
pnpm --filter <package-name> <script>
```

## forgod 게임 명령어

```bash
# 개발
pnpm forgod:dev          # forgod 개발 서버 (Vite HMR)
pnpm forgod:server:dev   # forgod API 서버 개발 모드 (watch)

# 빌드
pnpm forgod:build        # 전체 빌드 (server → forgod 순서)

# 테스트
pnpm forgod:test         # forgod-server 테스트 (게임 로직 + 서버)

# 서버
pnpm forgod:server       # forgod API 서버 시작

# 개별 패키지
pnpm --filter forgod dev
pnpm --filter @forgod/server build
pnpm --filter @forgod/server test
pnpm --filter @forgod/server start
```

## indian-poker 게임 명령어

```bash
# 개발
pnpm indian-poker:dev            # indian-poker 개발 서버 (Vite HMR)
pnpm indian-poker:server:dev     # indian-poker Socket.IO 서버 개발 모드 (watch)

# 빌드
pnpm indian-poker:build          # 전체 빌드 (server → indian-poker 순서)

# 테스트
pnpm indian-poker:test           # indian-poker-server 테스트 (게임 로직 + 서버)
pnpm indian-poker:e2e            # indian-poker UI E2E 테스트 (Playwright)

# 서버
pnpm indian-poker:server         # indian-poker 서버 시작 (포트 3002)

# 개별 패키지
pnpm --filter indian-poker dev
pnpm --filter indian-poker test:e2e
pnpm --filter @indian-poker/server build
pnpm --filter @indian-poker/server test
pnpm --filter @indian-poker/server start
```

## 새 게임 추가 시

1. `packages/<game>/` - 웹 UI 패키지 생성
2. `packages/<game>-core/` - 게임 로직 패키지 생성 (선택)
3. 루트 `package.json`에 편의 스크립트 추가 (`<game>:dev`, `<game>:build`, `<game>:test`)
4. 게임별 `AGENTS.md` 작성 (게임 규칙, 타입 구조 등)

## AI 캐릭터 이미지 생성

캐릭터 표정 이미지를 Vertex AI (Gemini)로 자동 생성하는 스크립트:

```bash
# 전체 표정 20개 생성
npx tsx scripts/generate-character-images.ts --character <character-id>

# 특정 표정만 생성
npx tsx scripts/generate-character-images.ts --character <character-id> --expressions poker_face,confident

# 기존 이미지 덮어쓰기
npx tsx scripts/generate-character-images.ts --character <character-id> --force
```

- 레퍼런스 이미지: `scripts/references/{character-id}.{jpg,png,webp}`
- 출력: `packages/indian-poker/public/characters/{character-id}/{expression}.png` (20개)
- 파이프라인: Gemini 이미지 생성 → rembg 배경 제거 → PIL 정사각형 패딩
- 필요: gcloud CLI 인증, Python (rembg, PIL)

## 기술 스택

- **Package Manager**: pnpm 9.15+
- **Build System**: Turborepo
- **언어**: TypeScript 5.0+
