// 패스 앤 슛 덱 v3 평가기: 성향 토큰 + 호흡/혼선 + 대칭 카운터 + 밴드 트랙 (구조·덱 모두 제안)
// 사용: node playtests/deck-v3-eval.js            통계와 검증
//       node playtests/deck-v3-eval.js --md       카드 목록·수치표 마크다운
//       node playtests/deck-v3-eval.js --opt      아이콘 배치 담금질 최적화 (ITERS, MAXT, HOP 환경변수)
//
// 모델:
...
//   특성별 v: 엇갈린 줄이면 뚫음 +(a+d), 같은 줄이면 잡힘 -(a+d), 한쪽만 ±세기. 힘겨루기 없음.
//   호흡: 두 장이 같은 성향 아이콘을 모두 갖고 있으면 그 특성 +HOP (기본 2). 혼선: 상쇄 쌍마다 -1. 텍스트 보너스 없음.
...
const HOP=parseInt(process.env.HOP||'5',10);
...
const ATT=[
 ...
 {id:'A02',name:'티키타카',grade:'S',icons:{점유:2,패턴:2,중앙:1,발밑:1},adj:['중앙','발밑'],desc:'짧은 패스로 상대를 끌어내고 공간을 만든다.'},
 {id:'A03',name:'롱볼',grade:'S',icons:{전환:2,뒷공간:2,개인:1},adj:['개인','중앙'],desc:'후방에서 한 번에 전방으로 보낸다.'},
 {id:'A04',name:'측면 오버로드',grade:'S',icons:{점유:2,측면:2,패턴:2},adj:['점유','패턴'],desc:'한쪽 측면에 수적 우위를 만든다.'},
 {id:'A05',name:'스위칭 플레이',grade:'S',icons:{측면:3,패턴:2,점유:2},adj:['점유','패턴'],desc:'긴 횡패스로 반대편으로 전환한다.'},
 {id:'A06',name:'중거리 슛',grade:'S',icons:{개인:2,중앙:2},adj:['중앙','발밑'],fx:'max',desc:'박스 밖에서 때린다. 한 방.'},
 {id:'A07',name:'템포 조절',grade:'S',icons:{점유:2,패턴:2},adj:['발밑','패턴'],fx:'safe',desc:'공을 돌리며 위험을 피한다. 카운터를 맞지 않는다.'},
 {id:'A08',name:'오버래핑 풀백',grade:'C',icons:{측면:3},adj:['전환','개인'],desc:'풀백이 윙어 바깥으로 올라가 폭을 만든다.'},
 {id:'A09',name:'얼리 크로스',grade:'C',icons:{측면:2,전환:2},adj:['뒷공간','전환'],desc:'수비가 자리 잡기 전에 일찍 올린다.'},
 {id:'A10',name:'컷백',grade:'C',icons:{측면:2,패턴:2},adj:['패턴','발밑'],desc:'엔드라인까지 가서 뒤로 내준다.'},
 {id:'A11',name:'하프스페이스 침투',grade:'C',icons:{중앙:2,패턴:2,발밑:1,점유:1},adj:['발밑','점유'],desc:'측면과 중앙 사이 공간으로 파고든다.'},
 {id:'A12',name:'스루패스',grade:'C',icons:{중앙:3,뒷공간:2},adj:['전환','패턴'],desc:'수비 라인 사이로 찔러 넣는다.'},
 {id:'A13',name:'인버티드 윙어',grade:'C',icons:{중앙:3,개인:2},adj:['발밑','전환'],desc:'윙어가 안쪽으로 접어 들어온다.'},
 {id:'A14',name:'세컨볼 회수',grade:'C',icons:{전환:2,뒷공간:2},adj:['개인','뒷공간'],desc:'떨어지는 공을 먼저 줍는다.'},
 {id:'A15',name:'세트피스 유도',grade:'C',icons:{개인:2,측면:1},adj:['측면','발밑'],desc:'파울을 끌어내 프리킥·코너로 간다.'},
 {id:'A16',name:'역습',grade:'X',icons:{전환:2,개인:1,뒷공간:1},adj:['개인','뒷공간'],desc:'공을 얻자마자 빠르게 앞으로.'},
 {id:'A17',name:'라인 브레이킹 런',grade:'X',icons:{뒷공간:2,전환:2,개인:2},adj:['전환','개인'],fx:'max',desc:'수비 라인 뒤로 달린다.'},
 {id:'A18',name:'개인 돌파',grade:'X',icons:{개인:2,발밑:1},adj:['측면','발밑'],fx:'max',desc:'한 명이 1대1로 제친다.'},
 {id:'A19',name:'타겟맨 활용',grade:'X',icons:{발밑:3,개인:2,전환:1},adj:['전환','중앙'],desc:'큰 공격수가 공을 지키고 떨궈 준다.'},
 {id:'A20',name:'폴스 나인',grade:'X',icons:{발밑:2,패턴:2,점유:1,중앙:1},adj:['점유','중앙'],desc:'공격수가 내려와 센터백을 끌어낸다.'},
];
const DEF=[
 {id:'D01',name:'하이 프레스',grade:'S',icons:{압박:2,대인:2,하이:2},adj:['하이','대인'],desc:'상대 진영에서부터 압박한다.'},
 {id:'D02',name:'미들 블록',grade:'S',icons:{밀집:2,지역:2,후퇴:2},adj:['후퇴','딥'],desc:'중원에 블록을 세우고 기다린다.'},
 {id:'D03',name:'로우 블록',grade:'S',icons:{후퇴:2,딥:2},adj:['딥','밀집'],desc:'자기 진영 깊숙이 내려선다.'},
 {id:'D04',name:'지역 수비',grade:'S',icons:{지역:2,밀집:2,딥:1},adj:['밀집','딥'],desc:'사람이 아니라 공간을 지킨다.'},
 {id:'D05',name:'맨마킹',grade:'S',icons:{대인:2,봉쇄:2,압박:1},adj:['압박','봉쇄'],desc:'사람을 따라붙는다.'},
 {id:'D06',name:'컴팩트',grade:'S',icons:{밀집:2,딥:1},adj:['지역','딥'],fx:'erase',desc:'간격을 좁혀 한 방을 지운다. 상대의 2d20 효과 무효.'},
 {id:'D07',name:'딥 라인',grade:'S',icons:{딥:2,지역:2,후퇴:1},adj:['후퇴','지역'],desc:'수비 라인을 내려 뒷공간을 없앤다.'},
 {id:'D08',name:'게겐프레싱',grade:'C',icons:{압박:2,대인:2,하이:1},adj:['하이','대인'],desc:'공을 잃은 직후 되찾는다.'},
 {id:'D09',name:'하이 라인',grade:'C',icons:{하이:2,압박:2,지역:1},adj:['압박','지역'],desc:'수비 라인을 올려 공간을 압축한다.'},
 {id:'D10',name:'오프사이드 트랩',grade:'C',icons:{하이:2,압박:1},adj:['지역','압박'],desc:'라인을 맞춰 올려 침투를 걸어 낸다.'},
 {id:'D11',name:'스위퍼 키퍼',grade:'C',icons:{하이:3,압박:1},adj:['압박','지역'],desc:'골키퍼가 나와 뒷공간을 정리한다.'},
 {id:'D12',name:'박스 밀집',grade:'C',icons:{딥:2,밀집:2,후퇴:1},adj:['지역','후퇴'],desc:'페널티 박스 안에 몸을 던진다.'},
 {id:'D13',name:'5백',grade:'C',icons:{봉쇄:3,지역:1,후퇴:2},adj:['후퇴','딥'],desc:'센터백 3명과 윙백으로 폭을 막는다.'},
 {id:'D14',name:'더블 피봇',grade:'C',icons:{밀집:2,후퇴:1},adj:['지역','후퇴'],desc:'수비형 미드필더 둘이 중앙을 잠근다.'},
 {id:'D15',name:'측면 봉쇄',grade:'C',icons:{봉쇄:3,대인:2,압박:2},adj:['대인','압박'],desc:'윙어를 바깥으로 몰고 더블팀한다.'},
 {id:'D16',name:'압박 트리거',grade:'X',icons:{압박:3,하이:2,지역:1},adj:['하이','지역'],desc:'백패스·횡패스 순간에만 달려든다.'},
 {id:'D17',name:'전술적 파울',grade:'X',icons:{대인:2,봉쇄:2},adj:['후퇴','봉쇄'],fx:'cut',desc:'위험해지기 전에 끊는다. 결정적 찬스를 주지 않는다.'},
 {id:'D18',name:'역습 세팅',grade:'X',icons:{후퇴:3,대인:1,딥:1},adj:['딥','대인'],desc:'뒤에 남았다가 뺏는 즉시 달린다.'},
 {id:'D19',name:'슛 스토퍼 GK',grade:'X',icons:{딥:3,밀집:1,후퇴:1},adj:['후퇴','밀집'],desc:'골키퍼가 라인을 지킨다.'},
 {id:'D20',name:'세트피스 수비',grade:'X',icons:{지역:3,밀집:1,딥:1},adj:['밀집','딥'],desc:'코너·프리킥에서 지역과 사람을 섞어 막는다.'},
];
...
function profile(cards,tIdx){const cnt=[[0,0],[0,0],[0,0],[0,0]];const bonus=[0,0,0,0];
  ...
  const shared=[0,0,0,0];for(let t=0;t<4;t++)for(let k=0;k<2;k++){const n=(tIdx===tIdxA?AT:DT)[t][k];if(cards.every(c=>(c.icons[n]||0)>0))shared[t]=1;}
  return cnt.map(([x,y],t)=>{const cancel=Math.min(x,y);return {side:x>y?0:y>x?1:-1,str:Math.abs(x-y),cancel,bonus:bonus[t]+shared[t]*HOP};});}
const P1=Array.from({length:21},(_,k)=>k?1/20:0),PMAX=Array.from({length:21},(_,k)=>k?(2*k-1)/400:0);
const CASCADE=process.env.CASCADE!=='0';const VSCALE=parseFloat(process.env.VSCALE||'2');
...
function objective(R){let pen=0;const gA=gradeStats(ATT,R.cardA),gD=gradeStats(DEF,R.cardD);
  ...
  pen+=2*Math.max(0,20.5-sd(R.VA))**2+2*Math.max(0,20.5-sd(R.VD))**2;
...
const MAXB=parseInt(process.env.MAXB||'0',10),MAXT=parseInt(process.env.MAXT||'7',10);
...
function optimize(iters){let R=evaluate();let cur=objective(R);let best=cur;let bestState=JSON.stringify([...ATT,...DEF].map(c=>[c.icons,c.bonus]));const all=[...ATT,...DEF];
  for(let it=0;it<iters;it++){const c=all[Math.floor(Math.random()*all.length)];const m=randomMove(c);if(!m)continue;apply(c,m,1);
    const r=evaluate();const o=objective(r);const T=Math.max(0.02,1.5*(1-it/iters));
