...
interface GameState {
  ...
  clones: Array<{ playerId: string; position: HexCoord }>  // 법사 분신
  monsterRoundBuffs: {
...
interface Player {
  ...
  knowsDemonSwordPosition: boolean
  isDead: boolean
  ...
  // 버프/상태 필드
  ironStanceActive: boolean      // 무적 태세 활성
...
2. **action 페이즈**
   - `BASIC_ATTACK`: 인접 대상 공격 (힘 수치만큼 피해). 턴당 1회만 가능
   - `USE_SKILL`: 스킬 사용
   - `ROLL_STAT_DICE`: 능력치 업그레이드
   - `COMPLETE_REVELATION`: 계시 완료
   - `END_TURN`: 턴 완료
   - `CHOOSE_CORRUPTION`: 타락 용사 처치 후 타락 여부 선택
   - `CHOOSE_ESCAPE_TILE`: 산/호수 탈출 시 목적지 선택

### 턴 시작 시 자동 처리
1. **산/호수 위치 체크**: 플레이어가 산/호수에 있으면 가장 가까운 이동 가능 타일로 자동 이동 (동거리 타일이 여러 개면 `CHOOSE_ESCAPE_TILE`로 선택)
2. **속박 해제**: `isBound` 상태면 이동력 0으로 설정 + 속박 해제
3. **무적 태세 해제**: `ironStanceActive` 해제
4. **화염 타일 피해**: 현재 위치가 화염이면 10 피해 (타락 면역)

### 몬스터 턴
1. 몬스터 주사위 6개 굴림 → 작은 순 정렬
2. 각 몬스터의 `diceIndices`가 가리키는 주사위들 합산
3. **몬스터 효과 발동** (조건 충족 시) - 공격과 **별개**
4. **몬스터 공격**: `주사위합 % 인접타일수` = 공격할 인접 타일 번호. **피해량 = 주사위합**
   - 효과와 공격은 **별개로 실행** (효과 먼저, 공격 항상 실행)
   - 트롤만 예외: 타락 용사에게는 홀수일 때만 공격
   - 은신 용사: 공격 스킵
   - 마검 소유자: 공격 스킵 (양방향 불가)
5. 발록 부활 체크 (희생 라운드 후)
...
| 화염 | 이동 경로 상 화염 타일을 밟을 때**마다** 10 피해 + 턴 시작 시 10 피해 (타락 용사 면역). 발록 사망 시 무효화 (발록 부활 시 다시 활성) |
| 신전 | 천사 계시 수행 장소. 타락 용사 진입 불가 (마검 보유 시 진입 가능, 피해 없음) |
| 마왕성 | 마왕 계시 수행 장소 |
| 산/호수 | 이동 불가. 강제 이동은 가능. 해당 플레이어 턴 시작 시 가장 가까운 이동 가능 타일로 자동 이동 (동거리 타일이 여러 개면 플레이어가 선택) |
...
| 무적 태세 | 2 | 3 | 다음 턴 시작까지 모든 피해를 힘 수치만큼 감소 (사용 턴 나머지 + 몬스터 턴 방어) |
| 검기 발사 | 3 | 3 | 앞 3칸에 힘 피해 |

### 도적 스킬
| 이름 | 비용 | 쿨타임 | 설명 |
|------|------|--------|------|
| 독 바르기 | 1 | 1 | 다음 기본 공격에 민첩 수치 추가 피해 (사용 후 소모) |
| 그림자 함정 | 1 | 2 | 현재 위치에 함정 설치 (최대 3개, 초과 시 가장 오래된 것 제거). 밟으면 함정 주인의 민첩만큼 피해 + 함정 주인 은신 활성 |
| 배후 일격 | 2 | 2 | 대상 뒤로 이동하며 민첩 피해 |
| 은신 | 2 | 3 | 모든 피해 면역 (몬스터 공격 포함). 본인이 공격하면 해제 |
| 무한의 표창 | 3 | 3 | 일직선 첫 대상에 민첩 피해. 2칸 이상이면 민첩×2 피해 |

### 법사 스킬
| 이름 | 비용 | 쿨타임 | 설명 |
|------|------|--------|------|
| 스킬 강화 | 2 | 0 | 다음 1회 스킬만 강화 (사용 후 소모) |
| 마법 화살 | 2 | 1 | 지능 피해 (용사 사거리 2). 강화: 분신도 공격 |
| 분신 | 2 | 2 | 현재 위치에 분신 생성 (플레이어당 1개). 분신은 지능 수치 이상의 피해를 받으면 즉시 파괴. 강화: 분신 위치로 순간이동 (위치 교환) |
| 마력 방출 | 3 | 2 | 주변 1칸에 지능 피해. 강화: 피해 후 대상 1칸 밀어냄 |
...
### 상태 전환
- **신성 → 타락**: 다른 용사를 **죽일 때** (비타락 용사 처치 시 자동 타락)
- **타락 → 신성**: **부활 시** 선택 가능
...
### 타락 용사 특징
- 신전 진입 불가 (마검 보유 시 가능, 피해 없음)
- 화염 타일 피해 면역
- 마왕성 진입 시 피해 없음

### 타락 용사 처치
**죽인 용사**가 처치 직후 즉시 타락 여부를 선택:
- `CHOOSE_CORRUPTION { accept: true }`: 타락 전환 + 타락 주사위 획득
- `CHOOSE_CORRUPTION { accept: false }`: 신성 유지
- ※ 비타락 용사를 처치하면 자동 타락 (선택 없음)
...
| 이름 | 체력 | 주사위 인덱스 | 효과 |
|------|------|---------------|------|
| 하피 | 20 | 1, 2, 3 | 합 ≥ 7: 인접 용사 한 칸 밀어냄 (몬스터에서 멀어지는 방향) |
| 그린딜로 | 25 | 2, 4 | 합 ≥ 7: 공격 대상 속박 (다음 턴 이동 불가) |
| 리치 | 30 | 4, 5, 6 | 합 ≥ 15: 몬스터들 메테오 면역 (다음 라운드까지) |
| 트롤 | 35 | 1, 6 | 타락 용사에게는 홀수일 때만 공격 |
| 히드라 | 40 | 1, 2, 5, 6 | 합 ≥ 15: 잃은 체력의 2배 회복 (maxHealth 초과 가능) |
| 골렘 | 50 | 5, 6 | 합 = 12: 기본 공격 피해 무시 (다음 라운드까지) |
| 발록 | 60 | 1, 2, 3, 4, 5, 6 | 사망 시 화염 타일 피해 무효화 (부활 시 다시 활성). 희생 라운드 후 자동 부활 |
...
### 이벤트 기반 계시 자동 완료
전투 중 조건이 충족되는 계시(발록 공격, 발록 처단, 선제 공격, 타락 유혹 등)는 **전투 직후 자동으로 완료 처리**됩니다.

### angel-7 천사의 가호 (constants.ts에 정의)
- 조건: 타락 용사에게 죽을만큼 피해를 받음
- 효과: **체력 1로 생존** + 계시 자동 완료
- 보상: 신앙 3점
...
**마검 특수 규칙**:
- 마검 소유자와 몬스터는 서로 싸울 수 없음 (양방향 불가: 소유자→몬스터 공격 불가, 몬스터→소유자 공격 안 함)
- 마검 소유자 사망 시 타락 용사가 시체 위에서 마검 획득 가능
- 마검 보유 시 타락 상태에서도 신전 진입 가능 (피해 없음)
