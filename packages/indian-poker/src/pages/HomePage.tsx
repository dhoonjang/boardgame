...
import { AI_CHARACTERS } from '@indian-poker/server/game'
...
export default function HomePage() {
  ...
  const [name, setName] = useState(() => localStorage.getItem('indian-poker-player-name') ?? '')
  ...
  const saveName = (n: string) => {
    localStorage.setItem('indian-poker-player-name', n)
  }
  ...
  const handleAIGame = () => {
    ...
    saveName(name.trim())
    createAIGame(name.trim(), selectedCharacter ?? undefined)
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          <h1 className="text-5xl font-extrabold text-poker-gold mb-2">Indian Poker</h1>
          <p className="text-slate-400">캐릭터와 함께하는 2인 블러프 카드 게임</p>
        </div>
        ...
        {mode === 'menu' && (
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              disabled={!isConnected}
              className="w-full px-6 py-4 bg-poker-accent hover:bg-amber-600 disabled:bg-slate-700 text-white font-semibold rounded-xl text-lg transition-colors"
            >
              방 만들기
            </button>
            <button
              ...
              disabled={!isConnected}
              className="w-full px-6 py-4 bg-poker-surface hover:bg-slate-600 disabled:bg-slate-700 text-white font-semibold rounded-xl text-lg transition-colors border border-poker-border"
            >
              방 참가하기
            </button>
          </div>
        ...
        {mode === 'ai' && (
          <div className="flex flex-col gap-3">
            <input
              ...
              maxLength={10}
              className="w-full px-4 py-3 bg-poker-surface border border-poker-border rounded-lg text-white placeholder-slate-500 focus:outline-none focus:border-poker-accent"
              autoFocus
            />
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg border text-left transition-colors ${
                    selectedCharacter === null
                      ...
                      : 'border-poker-border bg-poker-surface text-slate-400 hover:border-slate-500'
                  }`}
                >
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <img
                        src={c.avatarUrl}
                        alt={c.name}
                        className="w-8 h-8 rounded-full object-cover bg-poker-bg"
                    ...
                    <div className="mt-1">
                      <span className={`text-xs px-1.5 py-0.5 rounded ${
                        ...
                        c.difficulty === 'normal' ? 'bg-amber-500/20 text-amber-300' :
                        'bg-red-500/20 text-red-300'
                      }`}>
                        {c.difficulty === 'easy' ? '쉬움' : c.difficulty === 'normal' ? '보통' : '어려움'}
                      </span>
                    </div>
                  </button>
                ))}
        ...
        {mode === 'create' && (
          <div className="flex flex-col gap-3">
            ...
            <button
              ...
              disabled={!isConnected}
              className="w-full px-6 py-3 bg-poker-accent hover:bg-amber-600 disabled:bg-slate-700 text-white font-semibold rounded-lg transition-colors"
            >
              게임 만들기
            </button>
            <button
        ...
        {mode === 'join' && (
          <div className="flex flex-col gap-3">
            ...
            <input
              ...
              maxLength={4}
              className="w-full px-4 py-3 bg-poker-surface border border-poker-border rounded-lg text-white placeholder-slate-500 focus:outline-none focus:border-poker-accent text-center text-2xl tracking-widest font-mono"
            />
            <button
              onClick={handleJoin}
              disabled={!isConnected}
        ...
        {/* 게임 규칙 요약 */}
        <div className="mt-8 bg-poker-surface rounded-xl border border-poker-border p-4">
