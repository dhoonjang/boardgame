import type { PlayerView, RoundResult, Card } from '@indian-poker/server/game'
...
export default function GameOverModal({ view, myPlayerId, onGoHome, onContinueChat, isAIGame }: Props) {
      ...
      <div className="bg-poker-surface border border-poker-border rounded-2xl p-8 max-w-sm w-full mx-4 text-center">
        <h2 className="text-3xl font-extrabold mb-4">
          {draw ? (
            ...
            <span className="text-poker-gold">승리!</span>
          ) : (
        ...
        <div className="flex justify-center gap-8 mb-6">
          <div className="text-center">
            ...
            <p className="text-2xl font-bold text-poker-gold">{view.me.chips}</p>
          </div>
          <div className="text-slate-500 text-2xl font-bold self-end">vs</div>
          <div className="text-center">
            <p className="text-sm text-slate-400">{view.opponent.name}</p>
            <p className="text-2xl font-bold text-poker-gold">{view.opponent.chips}</p>
          </div>
        </div>

        <div className="flex flex-col gap-2">
          {isAIGame && onContinueChat && (
            <button
              onClick={onContinueChat}
              className="w-full px-6 py-3 bg-poker-accent hover:bg-amber-600 text-white font-semibold rounded-lg transition-colors"
            >
              계속 대화하기
            </button>
          )}
          <button
            ...
            className={`w-full px-6 py-3 font-semibold rounded-lg transition-colors ${
              isAIGame
                ...
                : 'bg-poker-accent hover:bg-amber-600 text-white'
            }`}
          >
            홈으로
          </button>
