...
export class GameEngine {
  ...
  private startRound(state: GameState): ActionResult {
    ...
    // 덱 준비: 기존 덱에서 소비, 2장 미만이면 새 20장 덱 생성
    const isNewDeck = state.deck.length < 2
    const deck = isNewDeck
      ? this.shuffler.shuffle([...FULL_DECK])
      : [...state.deck]
    ...
    const carryOverPot = state.pot // 무승부 이월 팟
    const totalPot = carryOverPot + ante0 + ante1

    const events: GameEvent[] = [{
      type: 'ROUND_START',
      message: carryOverPot > 0
        ? `라운드 ${newRoundNumber} 시작! 이월 ${carryOverPot} + 앤티 ${ante0 + ante1} = ${totalPot}칩.`
        : `라운드 ${newRoundNumber} 시작! 앤티 ${ante0 + ante1}칩.`,
  ...
  private getBettingActions(state: GameState, player: Player): ValidAction[] {
    ...
    // 레이즈 (칩이 callAmount + 1 이상 있어야, 상대 잔액 이내)
    if (player.chips > callAmount && opponent.chips > 0) {
      const maxRaise = Math.min(player.chips - callAmount, opponent.chips)
      if (maxRaise >= MIN_RAISE_AMOUNT) {
        actions.push({
          type: 'RAISE',
