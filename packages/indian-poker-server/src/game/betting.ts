...
import { FOLD_TEN_PENALTY } from './constants'
...
export function executeRaise(state: GameState, playerId: string, amount: number): ActionResult {
  ...
  // 상대 잔액 초과 레이즈 불가
  if (amount > opponent.chips) {
    return { success: false, newState: state, message: '상대 잔액을 초과하여 레이즈할 수 없습니다.', events: [] }
  }
...
export function executeFold(state: GameState, playerId: string): ActionResult {
  ...
  // 10 폴드 패널티: 카드가 10이면 추가 패널티
  const penalty = player.card === 10 ? Math.min(FOLD_TEN_PENALTY, player.chips) : 0

  const newPlayers = [...state.players] as [typeof state.players[0], typeof state.players[1]]
  newPlayers[playerIndex] = { ...player, hasFolded: true, chips: player.chips - penalty }

  // 상대가 팟 + 패널티 획득
  newPlayers[1 - playerIndex] = {
    ...opponent,
    chips: opponent.chips + state.pot + penalty,
  }
  ...
  const roundResult = {
    ...
    player0ChipChange: playerIndex === 0 ? -player.currentBet - penalty : state.pot + penalty - opponent.currentBet,
    player1ChipChange: playerIndex === 1 ? -player.currentBet - penalty : state.pot + penalty - opponent.currentBet,
    ...(penalty > 0 ? { penalty } : {}),
  }
  ...
  const events: GameEvent[] = [{
    ...
    playerId,
    message: penalty > 0
      ? `${player.name}이(가) 폴드했습니다. 10 패널티 ${penalty}칩! ${opponent.name}이(가) ${state.pot + penalty}칩을 획득합니다.`
      : `${player.name}이(가) 폴드했습니다. ${opponent.name}이(가) ${state.pot}칩을 획득합니다.`,
  }]
  ...
  return {
    ...
    newState,
    message: penalty > 0
      ? `폴드했습니다. 10 패널티 ${penalty}칩! ${opponent.name}이(가) 팟 + 패널티를 획득합니다.`
      : `폴드했습니다. ${opponent.name}이(가) 팟을 획득합니다.`,
    events,
  }
}
