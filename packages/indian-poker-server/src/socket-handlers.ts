...
export function registerSocketHandlers(io: Server, sessionManager: SessionManager): void {
  io.on('connection', (socket: Socket) => {
    ...
    // ─── create-ai-game ───
    socket.on('create-ai-game', (data: { player: { id: string; name: string }; characterId?: string }) => {
      const { player, characterId } = data
      ...
      // 2. AI 플레이어 생성 및 참가
      const aiPlayer = new AIPlayer(gameId, io, socket.id, sessionManager, characterId)
      ...
      socket.emit('opponent-joined', { opponentName: aiPlayer.name, characterId: aiPlayer.characterId })

      console.log(`[AI Game] 게임 ${gameId} 생성 — AI 캐릭터: ${aiPlayer.name} (${aiPlayer.characterId})`)
    })
    ...
    socket.on('game-action', (data: { action: GameAction }) => {
      ...
      // PvP 게임에서 round_end/game_over 시 즉시 전송
      if (!isAI && (result.newState.phase === 'round_end' || result.newState.phase === 'game_over')) {
        const lastRound = result.newState.roundHistory[result.newState.roundHistory.length - 1]
        if (lastRound) {
          for (const [, socketId] of room.players) {
            io.to(socketId).emit('show-round-result', { roundResult: lastRound })
          }
        }
        if (result.newState.phase === 'game_over') {
          for (const [, socketId] of room.players) {
            io.to(socketId).emit('show-game-over')
          }
        }
      }
    ...
    socket.on('player-chat', (data: { message: string }) => {
      ...
      if (sessionManager.isAIGame(gameInfo.gameId)) {
        // AI 게임: 기존 로직 유지
        ...
        // PvP 게임: 상대에게 메시지 전달
        const room = sessionManager.getRoom(gameInfo.gameId)
        if (!room) return
        const senderPlayerId = sessionManager.getPlayerIdBySocketId(socket.id)
        const senderName = gameInfo.room.gameState.players.find(p => p.id === senderPlayerId)?.name ?? '?'
        for (const [, socketId] of room.players) {
          if (socketId !== socket.id) {
            io.to(socketId).emit('player-message', { senderName, message: data.message })
          }
        }
      }
    })
